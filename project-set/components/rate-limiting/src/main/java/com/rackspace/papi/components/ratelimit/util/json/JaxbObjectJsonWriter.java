...
public class JaxbObjectJsonWriter {
   ...
   public void writeField(Field f) throws IOException {
      ...
      try {
         writeComplexField(f, callGetter(getterMethodName, objectBeingWritten, objectClass));
      } catch (GetterMethodNotFoundException gmnfe) {
         throw new ReflectionException(gmnfe.getMessage(), gmnfe);
      }
   }

   public void writeComplexField(Field f, Object fieldValue) throws IOException, GetterMethodNotFoundException {
      if (fieldValue != null) {
         final String xmlName = getJaxbNameForField(f);
         jsonGenerator.writeFieldName(xmlName);
         
         final Class fieldClass = fieldValue.getClass();

         if (Enum.class.isAssignableFrom(fieldClass)) {
            writeSimpleObject(callGetter("value", fieldValue, fieldClass));
   ...
   public void writeSimpleObject(Object fieldValue) throws IOException, GetterMethodNotFoundException {
      final Class fieldClass = fieldValue.getClass();
   ...
   public String formatGetterMethodName(String name, Class fieldType) {
      ...
      // JAXB espcaes field names that clash with a '_' character
